import { Folder, Check, User } from 'lucide-react';
...
export function ProfileSelector({
  ...
  if (profiles.length === 0) {
    return (
      <EmptyState
        ...
        description="Please ensure Firefox is installed and has at least one profile."
      />
    );
  }
  ...
  return (
    <div className="bg-card border border-border-subtle/50 rounded-xl p-6 shadow-sm">
      ...
      <div className="space-y-2" role="radiogroup" aria-label="Firefox Profile Selection">
        {profiles.map((profile) => {
          ...
          return (
            ...
            >
              ...
              <div className={`shrink-0 mr-3 flex items-center justify-center w-8 h-8 rounded-full transition-colors ${
                 isSelected ? 'bg-primary text-white' : 'bg-white/5 text-gray-500 group-hover:bg-white/10 group-hover:text-gray-300'
              }`}>
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  <span className={`text-sm font-medium truncate transition-colors ${isSelected ? 'text-primary' : 'text-gray-200'}`}>
                    {profile.name}
                  </span>
                  {profile.is_default && (
